<x-app-layout>
    ...
    <div class="py-6 max-w-3xl mx-auto bg-white p-6 shadow rounded">
        ...
        <div class="mt-4">
            <a href="{{ route('transactions.edit', $transaction) }}" class="inline-block bg-blue-500 text-black px-4 py-2 rounded mr-2">Redaguoti</a>
            ...
            <form action="{{ route('transactions.destroy', $transaction) }}" method="POST" class="inline-block" onsubmit="return confirm('Ar tikrai ištrinti?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="bg-red-500 text-black px-4 py-2 rounded">Ištrinti</button>
            </form>
        </div>
    </div>
</x-app-layout>
